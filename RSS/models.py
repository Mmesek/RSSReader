...
class Feed(FeedMeta, Base):
    ...
    components: List["Feed_Component"] = relationship(
        "Feed_Component", back_populates="feed", foreign_keys="Feed_Component.source"
    )
...
class Webhook(Base):
    ...
    subscriptions: List[Subscription] = relationship(
        "Subscription", back_populates="webhook", foreign_keys="Subscription.webhook_id"
    )
        ...
        for thread, groups in _group.threads.items():
            for group in groups:
                ...
                for embed in group.embeds:
                    if (
                        embed.total_characters + sum(i.total_characters for i in _embeds) < Limits.TOTAL
                        and len(_embeds) < 10
                    ):
